...
class Ferdinand(Restaurant):
    ...
    def parse(self, html):
        ...
        return [self.decode(re.sub('<[^<]+?>','',obed).replace("\t",' : ').lstrip(' ')) for obed in obedy[:4]]
...
class Flagship(Restaurant):
    ...
    def parse(self, html):
        ...
        try:
            ...
            return [(re.sub('<[^<]+?>','',obed).replace("\t",' : ').lstrip(' ')).decode("utf8") for obed in obedy[:5]]
        except:
            return []
...
class Club(ZomatoRestaurant):
    ...
    def parse(self, html):
        ...
        return [(re.sub('<[^<]+?>','',obed).replace("\t",' : ').lstrip(' ')).decode("utf8") for obed in obedy]
...
class Obyvacka(ZomatoRestaurant):
    ...
    def parse(self, html):
        obedy = re.findall('<div class="tmi-name">\n.*/', html, re.MULTILINE)
        return [(re.sub('<[^<]+?>','',obed).replace("\t",' : ').lstrip().rstrip()).decode("utf8") for obed in obedy[:5] if 'polievku,' not in obed]
